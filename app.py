...
if pagina == "Visão geral":
    ...
    st.write(
        "O conjunto de dados reúne registros de acidentes rodoviários "
        "fatais ocorridos na Austrália entre "
        f"{dados['Year'].min()} e {dados['Year'].max()}."
    )
    ...
    col4.metric(
        ...
        len(obter_colunas_numericas(dados))
        + len(obter_colunas_categoricas(dados))
    )

    st.subheader("Variáveis numéricas")

    st.write(
        ", ".join(obter_colunas_numericas(dados))
    )

    st.subheader("Variáveis categóricas")

    st.write(
        ", ".join(obter_colunas_categoricas(dados))
    )

    st.info(
        "A variável Crash ID é utilizada como identificador de cada "
        "registro e não é considerada uma variável numérica para "
        "as análises estatísticas."
    )
if pagina == "Estatística descritiva":
    ...
    tabela_frequencia.index = range(
        1,
        len(tabela_frequencia) + 1
    )
    ...
    ax.set_title(
        f"Distribuição de {variavel_escolhida}"
    )
    ...
    frequencias_categoricas = frequencia(
        valores_categoricos
    )
    ...
    tabela_categorica.index = range(
        1,
        len(tabela_categorica) + 1
    )
...
if pagina == "Simulações":
    ...
    st.write(
        "Nesta seção é possível observar, por meio de simulações, "
        "como alguns conceitos de probabilidade e estatística "
        "se comportam conforme a quantidade de repetições aumenta."
    )
    ...
    st.info(
        ...
        "a proporção observada tende a se aproximar de 50%, "
        "que é a probabilidade esperada de obter cara em uma moeda justa."
    )
    ...
    st.info(
        "O Teorema do Limite Central mostra que, "
        "à medida que várias amostras são coletadas, "
        "a distribuição das médias amostrais tende a assumir "
        "um formato aproximadamente Normal."
    )
...
if pagina == "Distribuições":
    ...
    st.write(
        "Nesta seção, os dados reais são comparados com "
    ...
    variancia_acidentes = variancia(
        acidentes_por_mes
    )

    razao_variancia_media = (
        variancia_acidentes / lambda_poisson
    )
    ...
    for k in k_poisson:
        ...
        quantidade = frequencias.get(
            k,
            0
        )

        frequencia = (
            quantidade / len(acidentes_por_mes)
        )

        frequencias_reais.append(
            frequencia
        )
    ...
    ax.set_xlabel(
        "Quantidade de acidentes no mês"
    )
    ...
    ax.set_title(
        "Acidentes mensais e distribuição de Poisson"
    )
    ...
    st.info(
        "A comparação entre os dados reais e a distribuição de Poisson "
        "permite avaliar visualmente se o modelo teórico representa "
        "adequadamente a quantidade de acidentes por mês."
    )


if pagina == "Correlação e regressão":
    ...
    if variavel_x == variavel_y:
        ...
        valores_x = dados_regressao[
            variavel_x
        ].tolist()

        valores_y = dados_regressao[
            variavel_y
        ].tolist()
        ...
        st.write(
            f"Equação da reta: "
            f"y = {inclinacao:.4f}x + {intercepto:.4f}"
        )
        ...
        st.metric(
            f"Valor previsto de {variavel_y}",
            f"{previsao:.0f}"
        )
        ...
        st.write(
            f"A correlação indica uma relação linear "
            f"{direcao} {intensidade} entre as variáveis."
        )

        if inclinacao > 0:

            st.write(
                f"Para cada aumento de 1 unidade em "
                f"{variavel_x}, o modelo prevê um aumento médio "
                f"de {inclinacao:.4f} unidades em {variavel_y}."
            )
            ...
            st.write(
                f"Para cada aumento de 1 unidade em "
                f"{variavel_x}, o modelo prevê uma redução média "
                f"de {abs(inclinacao):.4f} unidades em {variavel_y}."
            )

        else:

            st.write(
                f"O modelo não indica mudança linear em "
                f"{variavel_y} quando {variavel_x} aumenta."
            )

        st.write(
            f"Quando {variavel_x} é igual a 0, "
            f"o modelo prevê {intercepto:.4f} para {variavel_y}. "
            "Dependendo das variáveis escolhidas, esse valor pode "
            "não ter uma interpretação prática."
        )

        st.write(
            f"O modelo explica aproximadamente "
            f"{r2 * 100:.2f}% da variação da variável Y."
        )

        st.info(
            "A correlação e a regressão mostram apenas uma "
            "associação linear entre as variáveis. "
            "Isso não significa que uma variável cause a outra."
        )
...
if pagina == "Descobertas":
    st.header("Descobertas estatísticas")

    st.write(
        "Nesta seção são apresentadas três descobertas "
        "estatísticas encontradas no conjunto de dados."
    )

    st.subheader(
        "1. Registros fatais envolvendo homens e mulheres"
    )
    ...
    st.write(
        f"Isso representa aproximadamente "
        f"{razao_genero:.2f}".replace(".", ",")
    ...
    ax.set_title(
        "Registros fatais por gênero"
    )

    st.pyplot(fig)

    st.subheader(
        "2. A quantidade de registros varia entre os estados"
    )
    ...
    estado_maior = (
        quantidade_por_estado.idxmax()
    )

    quantidade_maior = (
        quantidade_por_estado.max()
    )

    estado_menor = (
        quantidade_por_estado.idxmin()
    )

    quantidade_menor = (
        quantidade_por_estado.min()
    )
    ...
    quantidade_por_idade = (
        dados["Age Group"].value_counts()
    )

    faixa_mais_comum = (
        quantidade_por_idade.idxmax()
    )

    quantidade_mais_comum = (
        quantidade_por_idade.max()
    )
    ...
    ax.set_title(
        "Registros fatais por faixa etária"
    )
